...
We first generate a bunch of deviates and then plot their density in teal Then the true density is overlayed as a black line.
...
ggplot(df_uniform_samples, aes(x = u)) + 
  ...
  stat_function(aes(x = 0), fun = dunif, args = list(min = 0, max = 1)) + 
  labs(x = "X", 
       y = "Density") + 
  theme_minimal()
```
...
ggplot(df_beta_samples, aes(x = b)) + 
  geom_density(color = NA, fill = "#35a09c") + 
  stat_function(aes(x = 0), fun = dbeta, args = list(shape1 = 2, shape2 = 2)) + 
  labs(x = "X", 
       y = "Density") + 
  theme_minimal()
```
...
ggplot(df_custom_samples, aes(x = b)) + 
  geom_density(color = NA, fill = "#35a09c") + 
  stat_function(aes(x = 0), fun = dcustom) + 
  labs(x = "X", 
       y = "Density") + 
  theme_minimal()
```
